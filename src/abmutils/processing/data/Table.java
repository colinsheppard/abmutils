...
package abmutils.processing.data;
...
//  protected boolean awfulCSV = false;
  ...
  public Table(InputStream input, String options, String types) throws IOException {
    init();
    parse(input, options);
  }
...
//        }
  ...
  */
  ...
  public String toString(){
	  ...
	  for (int i = 0; i < Math.min(this.rowCount,5); i++) {
		  result += "\n";
		  for(int j = 0; j < this.columns.length; j++){
			  result += this.getString(i, j) + "\t";
		  }
	  }
	  if(this.rowCount > 5)result += "\n...";
	  return result;
  }
}